...
const MainLayout = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-950 transition-colors duration-500">
      {/* Sidebar */}
      <aside 
        className={`${isSidebarMinimized ? 'w-20' : 'w-72'} bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-[4px_0_24px_rgba(0,0,0,0.02)] dark:shadow-[4px_0_24px_rgba(0,0,0,0.2)] flex flex-col transition-all duration-300 ease-in-out border-r border-gray-200/50 dark:border-gray-800/50 z-20 relative`}
      >
        {/* Toggle Button */}
        <button 
          onClick={() => setIsSidebarMinimized(!isSidebarMinimized)}
          className="absolute -right-4 top-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full p-1.5 shadow-md text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 transition-all hover:scale-110 z-30 flex items-center justify-center w-8 h-8"
        >
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                to={item.path}
                className={
                  `group relative flex items-center ${isSidebarMinimized ? 'justify-center w-12 h-12 mx-auto px-0' : 'px-4 py-3.5'} rounded-2xl font-semibold transition-all duration-300 ${
                    isActive
              ...
              >
                ...
                {/* Sleek Tooltip for minimized state */}
                {isSidebarMinimized && (
                  <div className="absolute left-full ml-4 px-3 py-1.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-semibold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 whitespace-nowrap shadow-xl">
                    {item.name}
      ...
      <div className="flex-1 flex flex-col relative z-10">
        {/* Topbar */}
        <header className="h-20 bg-white/60 dark:bg-gray-900/60 backdrop-blur-md flex items-center justify-between px-8 z-10 transition-colors border-b border-gray-200/50 dark:border-gray-800/50">
          <div className="flex items-center">
            {isSidebarMinimized && (
              <button 
                onClick={() => setIsSidebarMinimized(false)}
                className="mr-4 p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 lg:hidden"
              >
                <Menu size={20} />
              </button>
            )}
          ...
          <div className="flex items-center space-x-5">
            ...
            <div className="flex items-center space-x-3 border-l border-gray-200 dark:border-gray-700 pl-5">
              <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 text-white flex items-center justify-center font-bold shadow-md cursor-pointer hover:shadow-lg hover:opacity-90 transition-all border-2 border-white dark:border-gray-800" title={user?.name}>
